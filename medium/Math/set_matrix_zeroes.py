...
class Solution:
    def setZeroes(self, matrix: List[List[int]]) -> None:
        ...
        for i in range(0,m):
            for j in range(0, n):
                if matrix[i][j]==0:
                    cellsi.add(i) # add the row index to the set
                    cellsj.add(j) # add the column index to the set
...
class Solution:
    def setZeroes(self, matrix: List[List[int]]) -> None:
        ...
        # Step 3: setting matrix values to zero if their starting values are zero
        for i in range(1, m):
            for j in range(1, n):
                if matrix[i][0]==0 or matrix[0][j]==0:
                    matrix[i][j]=0

        # Step 4: Handling the first row and column
        if firstRowImpacted:
            for col in range(n):
                matrix[0][col]=0
