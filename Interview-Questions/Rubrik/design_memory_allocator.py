...
class Allocator:
    ...
    def allocate(self, size: int, mID: int) -> int: # O(N)
        consecutiveFree = 0
        for i in range(self.n):
            if self.ma[i]==0:
                consecutiveFree+=1 # Calculating and checking if enough free space is available
            else:
                consecutiveFree = 0
            if consecutiveFree == size:
                start = i-size+1
                for j in range(start, i+1): # assigning free space to the avialable space
                    self.ma[j] = mID
                return start
        return -1
        

    def freeMemory(self, mID: int) -> int: # O(N)
        count = 0
        for i in range(self.n):
            if self.ma[i]==mID:
                self.ma[i]=0
